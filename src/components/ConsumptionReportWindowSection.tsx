...
const ConsumptionReportCard = ({ location, water, power, reportedBy }: any) => {
    ...
    return (
        <div style={{
            ...
            <div style={{ flex: 1 }}>
                ...
                {/* Data Fields */}
                <div style={fieldStyle}>
                    <span style={labelStyle}>Water Consumption:-</span>
                    <span>{water}</span>
                </div>

                <div style={fieldStyle}>
                    <span style={labelStyle}>Power Consumption:-</span>
                    <span>{power}</span>
                </div>

                <div style={fieldStyle}>
                    <span style={labelStyle}>Report By:-</span>
    ...
    const reports = [
        {
            ...
            reportedBy: 'Mr. ABC P XYZ | Maintenance Dept | 22 Hour Ago'
        },
        {
            location: 'Main Pagoda / Souvenir',
            water: '3,500 Liters (Tank B)',
            power: '1,250 kWh (Central A/C)',
            reportedBy: 'Mrs. Jane Smith | Electrical Dept | Yesterday'
        }
    ];
    ...
    return (
        <div style={{ backgroundColor: '#fff', minHeight: '100%', width: '100%', overflowY: 'auto', padding: '1rem' }}>
            {reports.map((report, idx) => (
                <ConsumptionReportCard key={idx} {...report} />
            ))}
        </div>
    );
};
